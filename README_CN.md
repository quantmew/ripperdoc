...
任务系统切换：
- 默认启用新的 Task Graph + Team 工具
- 设置 `RIPPERDOC_ENABLE_TASKS=false` 可回退到旧版 `TodoRead`/`TodoWrite`
- 默认按会话隔离任务列表（新会话默认干净；继续/恢复同一会话会保留任务）
- 如需跨会话共享任务列表，可设置 `RIPPERDOC_TASK_LIST_ID`
