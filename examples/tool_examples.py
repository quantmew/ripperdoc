...
async def example_bash_tool() -> None:
...
async def example_file_read_tool() -> None:
    """Example: Read a file."""
    ...
    async for result in tool.call(input_data, context):
        print(f"File: {result.data.file_path}")  # type: ignore[union-attr]
        print(f"Lines read: {result.data.line_count}")  # type: ignore[union-attr]
        print("Content preview:")
        print(result.data.content[:200] + "...")  # type: ignore[union-attr]


async def example_glob_tool() -> None:
    ...
    async for result in tool.call(input_data, context):
        print(f"Pattern: {result.data.pattern}")  # type: ignore[union-attr]
        print(f"Found {result.data.count} files")  # type: ignore[union-attr]
        if result.data.matches:  # type: ignore[union-attr]
            print("First 5 matches:")
            for match in result.data.matches[:5]:  # type: ignore[union-attr]
                print(f"  - {match}")


async def example_file_edit_tool() -> None:
...
async def example_query_workflow() -> None:
    ...
    async for result in glob_tool.call(glob_input, context):
        files = result.data.matches  # type: ignore[union-attr]
    ...
    if files:
        ...
        async for result in read_tool.call(read_input, context):
            print(f"First {result.data.line_count} lines:")  # type: ignore[union-attr]
            print(result.data.content)  # type: ignore[union-attr]
    ...
    async for result in bash_tool.call(bash_input, context):
        print(result.data.stdout)  # type: ignore[union-attr]
