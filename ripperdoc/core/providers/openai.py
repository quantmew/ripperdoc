...
def _extract_content_blocks_from_responses_payload(payload: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Extract text and tool_use blocks from Responses API payload."""
    blocks: List[Dict[str, Any]] = []
    output = payload.get("output")
    if not isinstance(output, list):
        return blocks
...
def _extract_from_codex_sse_events(
    events: List[Dict[str, Any]],
) -> tuple[str, Dict[str, int], Optional[Dict[str, Any]]]:
    """Extract text/usage/final response from Codex SSE events."""
    text_parts: List[str] = []
...
def _extract_unsupported_parameter_name(message: str) -> Optional[str]:
...
def _build_codex_oauth_tools(openai_tools: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Convert chat.completions function tool schema into responses/codex schema."""
...
def _build_codex_responses_input(
    ...
    for message in normalized_messages:
        ...
        text = _flatten_message_text(message.get("content"))
        if not text:
            continue
