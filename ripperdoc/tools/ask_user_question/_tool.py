...
async def prompt_single_choice_with_ui(
    ...
    try:
        prompt_choice = _root_callable("prompt_choice_async", prompt_choice_async)
        ...
        print(f"[ask_user_question_tool] Choice UI unavailable ({type(e).__name__}), using text mode.")
...
async def prompt_multi_choice_with_ui(
    ...
    while True:
        try:
            prompt_checkbox = _root_callable("prompt_checkbox_async", prompt_checkbox_async)
            selected = await prompt_checkbox(
                message=message,
                options=options,
                title=title,
                style_variant="ask_user_question",
...
async def collect_answers(
    ...
    while idx < total:
        ...
        prompt_answer = _root_callable("prompt_user_for_answer", prompt_user_for_answer)
        answer = await prompt_answer(
            question,
            idx + 1,
            total,
...
class AskUserQuestionTool(Tool[AskUserQuestionToolInput, AskUserQuestionToolOutput]):
    ...
    def needs_permissions(
        self,
        input_data: Optional[AskUserQuestionToolInput] = None,
    ) -> bool:
        return False
    ...
    def render_result_for_assistant(self, output: AskUserQuestionToolOutput) -> str:
        ...
        serialized = ", ".join(f'"{question}"="{answer}"' for question, answer in output.answers.items())
        return (
