...
from ripperdoc.utils.memory import AUTO_MEMORY_FILE_NAME
...
class MemoryTool(Tool[MemoryToolInput, MemoryToolOutput]):
    ...
    async def validate_input(
        ...
        try:
            ...
            if path_for_command is not None:
                resolve_path(path_for_command, root=root, allow_root=input_data.command == "view")
            ...
            if input_data.command == "rename":
                ...
                resolve_path(input_data.new_path, root=root, allow_root=False)
    ...
    def _memory_root(self) -> Path:
        return memory_root(self._memory_dir_override, self._project_path)
    ...
    def _view(self, input_data: MemoryToolInput) -> MemoryToolOutput:
        root = self._memory_root()
        target = resolve_path(self._path_for_command(input_data), root=root, allow_root=True)
        disp = self._display_path(target)

        if not target.exists():
            return MemoryToolOutput(
                command="view", success=False, path=disp,
                message=f"Path not found: {disp}",
            )
        ...
        if target.is_file():
            ...
            return MemoryToolOutput(
                command="view", success=True, path=disp,
                line_count=line_count, content=content,
        ...
        if not entries:
            return MemoryToolOutput(
                command="view", success=True, path=disp, entries=[],
                message=f"Memory directory is empty: {disp}",
            )

        return MemoryToolOutput(
            command="view", success=True, path=disp, entries=entries,
            message=f"Listed {len(entries)} memory entries under {disp}",
        )

    def _create(self, input_data: MemoryToolInput) -> MemoryToolOutput:
        root = self._memory_root()
        target = resolve_path(self._path_for_command(input_data), root=root, allow_root=False)
        disp = self._display_path(target)
        if target.exists():
            return MemoryToolOutput(
                command="create", success=False, path=disp,
                message=f"Path already exists: {disp}",
            )

        content = self._create_text(input_data) or ""
        write_text(target, content)
        return MemoryToolOutput(
            command="create", success=True, path=disp,
            line_count=len(content.splitlines()),
            message=f"Created memory file: {disp}",
        )
    ...
    def _str_replace(self, input_data: MemoryToolInput) -> MemoryToolOutput:
        ...
        if not target.exists() or not target.is_file():
            return MemoryToolOutput(
                command="str_replace", success=False, path=disp,
                message=f"Memory file not found: {disp}",
            )
        ...
        if occurrences == 0:
            return MemoryToolOutput(
                command="str_replace", success=False, path=disp,
                message=f"String not found in {disp}",
            )
        if occurrences > 1 and not input_data.replace_all:
            return MemoryToolOutput(
                command="str_replace", success=False, path=disp,
                message=(
                    f"String appears {occurrences} times in {disp}. "
        ...
        return MemoryToolOutput(
            command="str_replace", success=True, path=disp,
            replacements_made=replacements,
            line_count=len(updated.splitlines()),
            message=f"Updated {disp} ({replacements} replacement(s)).",
        )
    ...
    def _insert(self, input_data: MemoryToolInput) -> MemoryToolOutput:
        ...
        if not target.exists() or not target.is_file():
            return MemoryToolOutput(
                command="insert", success=False, path=disp,
                message=f"Memory file not found: {disp}",
            )
        ...
        if insert_line > max_insert_line:
            return MemoryToolOutput(
                command="insert", success=False, path=disp,
                message=(
                    f"insert_line {insert_line} is out of range for {disp}. "
        ...
        return MemoryToolOutput(
            command="insert", success=True, path=disp,
            line_count=len(updated.splitlines()),
            message=f"Inserted content at line {insert_line} in {disp}.",
        )
    ...
    def _delete(self, input_data: MemoryToolInput) -> MemoryToolOutput:
        ...
        if not target.exists():
            return MemoryToolOutput(
                command="delete", success=False, path=disp,
                message=f"Path not found: {disp}",
            )

        msg = delete_path(target)
        return MemoryToolOutput(
            command="delete", success=True, path=disp, message=msg,
        )

    def _rename(self, input_data: MemoryToolInput) -> MemoryToolOutput:
        root = self._memory_root()
        source = resolve_path(self._path_for_command(input_data), root=root, allow_root=False)
        target = resolve_path(input_data.new_path, root=root, allow_root=False)
        source_display = self._display_path(source)
        target_display = self._display_path(target)

        if not source.exists():
            return MemoryToolOutput(
                command="rename", success=False, path=source_display,
        ...
        return MemoryToolOutput(
            command="rename", success=True, path=source_display,
            new_path=target_display,
