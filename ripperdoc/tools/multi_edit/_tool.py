    ...
    async def call(
        ...
        if not existing:
            ...
            write_encoding = select_write_encoding(
                file_encoding, updated_content, resolved_path, log_prefix="[multi_edit_tool]",
            )
            ...
            try:
                ...
                output = MultiEditToolOutput(
                    ...
                    replacements_made=0,
                    success=False,
                    message="File was created while preparing edits. Read it first before attempting to edit it.",
                    applied_edits=applied,
                    created=False,
                )
                yield ToolResult(
                ...
                output = MultiEditToolOutput(
                    file_path=str(resolved_path),
                    replacements_made=0, success=False,
                    message=f"Error writing file: {exc}",
                    applied_edits=applied, created=True,
                )
                yield ToolResult(
                    data=output, result_for_assistant=self.render_result_for_assistant(output)
                )
            ...
            safe_record_snapshot(
                cache_key, updated_content, file_state_cache, log_prefix="[multi_edit_tool]",
            )
            ...
            output = MultiEditToolOutput(
                ...
                success=True,
                message=(f"Applied {len(applied)} edit(s) with {total_replacements} replacement(s) "
                         f"to {resolved_path}"),
                additions=additions, deletions=deletions,
                diff_lines=diff_lines, diff_with_line_numbers=[],
                applied_edits=applied, created=True,
            )
        ...
        try:
            with open_locked_file(resolved_path, file_encoding) as (
                handle, pre_lock_mtime, post_lock_mtime,
            ):
                if pre_lock_mtime is not None and post_lock_mtime is not None:
                    if post_lock_mtime > pre_lock_mtime:
                        output = MultiEditToolOutput(
                            ...
                            applied_edits=applied, created=False,
                        )
                        yield ToolResult(
                            data=output, result_for_assistant=self.render_result_for_assistant(output),
                        )
                ...
                try:
                    ...
                    output = MultiEditToolOutput(
                        file_path=str(resolved_path),
                        replacements_made=0, success=False,
                        message=str(exc), applied_edits=applied, created=False,
                    )
                    yield ToolResult(
                        data=output, result_for_assistant=self.render_result_for_assistant(output),
                    )
                ...
                write_error = atomic_write_with_fallback(
                    handle, resolved_path, updated_content, write_encoding,
                    original_content, temp_prefix=".ripperdoc_multi_edit_",
                ...
                if write_error:
                    output = MultiEditToolOutput(
                        file_path=str(resolved_path),
                        replacements_made=0, success=False,
                        message=write_error, applied_edits=applied, created=False,
                    )
                    yield ToolResult(
                        data=output, result_for_assistant=self.render_result_for_assistant(output),
                    )
        ...
        output = MultiEditToolOutput(
            ...
            additions=additions, deletions=deletions,
            diff_lines=diff_lines, diff_with_line_numbers=[],
            applied_edits=applied, created=created,
        )
