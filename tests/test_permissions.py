...
def test_plan_mode_auto_allows_default_permission_prompts(tmp_path: Path):
    """Plan mode should block non-read-only tools outside the active plan file."""
    checker = make_permission_checker(
        tmp_path,
        yolo_mode=False,
    ...
    assert "Plan mode is read-only" in result.message


def test_plan_mode_allows_write_to_active_plan_file(tmp_path: Path):
    """Plan mode should allow writing the dedicated plan file without prompting."""
    ...
    result = asyncio.run(
        checker(
            FileWriteTool(),
            FileWriteToolInput(file_path=str(plan_file), content="# plan\n"),
        )
    )
...
def test_plan_mode_blocks_bash_even_for_read_only_command(tmp_path: Path) -> None:
    """Plan mode should deny Bash entirely."""
    checker = make_permission_checker(
        tmp_path,
        yolo_mode=False,
        permission_mode="plan",
